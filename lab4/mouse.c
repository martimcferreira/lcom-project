...
void (mouse_ih)() {
    ...
    if (util_sys_inb(KBC_STAT_REG, &status) != 0) { mouse_error = true; return; }

    // Verifica se há dados (OBF) E se esses dados vêm do Rato (BIT(5) ou AUX)
    if ((status & KBC_OBF) && (status & BIT(5))) { 
        if (util_sys_inb(KBC_OUT_BUF, &mouse_byte) != 0) { mouse_error = true; return; }
        // Verifica erros de paridade ou timeout
        if (status & (KBC_PARITY | KBC_TIMEOUT)) mouse_error = true;
    } else {
        mouse_error = true;
    }
}
...
int (mouse_write_command)(uint8_t command) {
    ...
    while (max_retries > 0) {
        if (kbc_write_command(0xD4) != 0) { max_retries--; continue; }
        if (kbc_write_argument(command) != 0) { max_retries--; continue; }
        if (kbc_read_response(&ack_byte) != 0) { max_retries--; continue; }
